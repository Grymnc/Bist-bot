...
def hisse_analiz_et(ticker: str) -> dict:
    ...
    hacim_orani = float(df['Volume_Ratio'].iloc[-1])
...
def tarama_yap():
    ...
    for ticker in CONFIG["hisseler"]:
        try:
            ...
            hacim_str = "{:.1f}x".format(sonuc['hacim_orani'])
            mesaj = (
                donus_prefix +
                "Hisse: " + sonuc['ticker'] + "\n" +
                "Tarih: " + sonuc['tarih'] + "\n" +
                ...
                "Hacim Orani: " + hacim_str
            )
            log.info(mesaj.replace("<b>", "").replace("</b>", ""))

            # Dönüş sinyalinde doğruluk şartı yok, direkt gönder
            # Normal sinyalde %65+ doğruluk şartı var
            if donus:
    ...
    # Tarama sonu özet: dönüş sinyalleri (doğruluk şartı yok)
    if donus_sinyalleri:
        ozet = "DONUS SINYALLERI (SAT->AL)\n\n"
        for s in donus_sinyalleri:
            ozet += s["ticker"] + " - " + str(s["son_fiyat"]) + " TL - Al Olasiligi: " + "{:.1%}".format(s["al_olasiligi"]) + " - Dogruluk: " + "{:.1%}".format(s["model_dogrulugu"]) + "\n"
