...
def tarama_yap():
    ...
    for ticker in CONFIG["hisseler"]:
        try:
            ...
            mesaj = (
                f"{'🔄 DÖNÜŞ SİNYALİ!' + chr(10) if donus else ''}"
                f"📊 <b>{sonuc['ticker']}</b>
"
                f"Tarih: {sonuc['tarih']}
"
...
"
                f"Sinyal: {sonuc['sinyal']}{' (Önceki: SAT 🔴)' if donus else ''}
"
...
"
    ...
    if donus_sinyalleri:
        ozet = "🔄 <b>DÖNÜŞ SİNYALLERİ (SAT→AL)</b>

"
        for s in donus_sinyalleri:
            if s['model_dogrulugu'] >= 0.65:
                ozet += f"✅ {s['ticker']} — {s['son_fiyat']} TL — Al Olasılığı: {s['al_olasiligi']:.1%}
"
